...
class SD3Backbone(Backbone):
    def __init__(
      ...
      cache_attentions=None,
        ...
        if self.use_learnable_tokens:
            ...
            # assert num_tokens < 77, "num_tokens must be less than 77"
            ...
            # initialize self.learnable_tokens weight
            self.prompt_tokens.data = prompt_embeds[0, :, :]
            self.prompt_tokens.requires_grad = True
            self.pooled_prompt_embeds = pooled_prompt_embeds
            print("Learnable tokens initialized with prompt embedding.")
        else:
    ...
    def output_shape(self):
        return {
            "attn_cache": ShapeSpec(channels=1, stride=16),
            "feat_cache": ShapeSpec(channels=1536, stride=16),
            }
        ...
        with torch.no_grad():
            ...
            for l, blk in enumerate(self.transformer.transformer_blocks):
                if blk.attn.processor.attn_cache is not None:
                    blk.attn.processor.attn_cache = []
                if blk.attn.processor.feat_cache is not None:
                    blk.attn.processor.img_feat_cache = []
                    blk.attn.processor.txt_feat_cache = []
