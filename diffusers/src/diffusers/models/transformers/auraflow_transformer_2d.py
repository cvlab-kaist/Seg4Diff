...
class AuraFlowSingleTransformerBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor, temb: torch.FloatTensor, return_auxiliary: bool = False,):
        ...
        if return_auxiliary:
            aux_output["sim_hs"] = torch.einsum("bsd,bsd->bs", F.normalize(hidden_states + attn_output, dim=-1), F.normalize(hidden_states, dim=-1))
...
class AuraFlowJointTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if return_auxiliary:
            aux_output["sim_c_hs"] = torch.einsum("bsd,bsd->bs", F.normalize(encoder_hidden_states + context_attn_output, dim=-1), F.normalize(encoder_hidden_states, dim=-1))#.cpu().numpy()
        ...
        if return_auxiliary:
            return encoder_hidden_states, hidden_states, aux_output
...
class AuraFlowTransformer2DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        output = []
        output_aux = [] if return_aux else None
            ...
            if self.training and self.gradient_checkpointing:
                ...
                if return_aux:
                    encoder_hidden_states, hidden_states, aux = block(
                        ...
                        temb=temb,
                        return_auxiliary=return_aux
                    )
                    output_aux.append(aux)
                else:
                    encoder_hidden_states, hidden_states = block(
                        hidden_states=hidden_states, 
                        encoder_hidden_states=encoder_hidden_states, 
                ...
                att = block.attn.processor.attn_cache[0]
                
                if softmax_weights is not None:
                    att[:, -4096:, -4096:] *= softmax_weights[0] # i2i
                    att[:, -4096:, :-4096] *= softmax_weights[1] # i2t
                    att[:, :-4096, -4096:] *= softmax_weights[2] # t2i
                    att[:, :-4096, :-4096] *= softmax_weights[3] # t2t
                    
                
                if indices is not None:
                    #att_slice = att[:, :-154 + num_tokens, :-154 + num_tokens]
                    att_slice = att #[:, indices][..., indices]
                    
                    # val = torch.tensor(att_slice.shape[-1] - 4096, device=att.device)
                    # k = -att_slice[..., 4096:].logsumexp(dim=-1, keepdim=True) + att_slice[..., :4096].logsumexp(dim=-1, keepdim=True) + (val / 4096).log()
                    # #import pdb; pdb.set_trace()
                    # att_slice[..., 4096:] = att_slice[..., 4096:] + k
                    # att_slice = att_slice.softmax(dim=-1).mean(dim=0)
                    ...
                    output.append(att_slice)
                    block.attn.processor.attn_cache = []
        ...
        if len(self.single_transformer_blocks) > 0:
            ...
            for index_block, block in enumerate(self.single_transformer_blocks):
                if self.training and self.gradient_checkpointing:
                    ...
                    if return_aux:
                        combined_hidden_states, aux = block(
                    ...
                    if softmax_weights is not None:
                        att[:, -4096:, -4096:] *= softmax_weights[0] # i2i
                        att[:, -4096:, :encoder_seq_len] *= softmax_weights[1] # i2t
                        att[:, :encoder_seq_len, -4096:] *= softmax_weights[2] # t2i
                        att[:, :encoder_seq_len, :encoder_seq_len] *= softmax_weights[3] # t2t
                    
                    if indices is not None:
                        #att_slice = att[:, :-154 + num_tokens, :-154 + num_tokens]
                        att_slice = att #[:, indices][..., indices]
                        #import pdb; pdb.set_trace()
                        #k = att[:, indices, 4096:].logsumexp(dim=-1, keepdim=True) - att_slice[..., 4096:].logsumexp(dim=-1, keepdim=True)
                        val = torch.tensor(att_slice.shape[-1] - 4096, device=att.device)
                        k = -att_slice[..., 4096:].logsumexp(dim=-1, keepdim=True) + att_slice[..., :4096].logsumexp(dim=-1, keepdim=True) + (val / 4096).log()
                        #import pdb; pdb.set_trace()
                        att_slice[..., 4096:] = att_slice[..., 4096:] + k
        ...
        if indices is not None:
            return torch.stack(output, dim=0).unsqueeze(0) #, output_aux
