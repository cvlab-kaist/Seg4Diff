...
def _get_coco_object_meta():
    stuff_ids = [k["id"] for k in COCO_CATEGORIES if "isthing" in k.keys()]
    assert len(stuff_ids) == 80, len(stuff_ids)
...
def register_all_coco_object_10k(root):
    #root = os.path.join(root, "coco-stuff")
    root = os.path.join(root, "COCO2014")
    meta = _get_coco_object_meta()
    for name, image_dirname, sem_seg_dirname in [
        ("train", "train2014", "annotations_detectron2/train2014"),
        ("train_wo_bg", "train2014", "annotations_detectron2/train2014"),
        ("test", "val2014", "annotations_detectron2/val2014"),
        ("test_wo_bg", "val2014", "annotations_detectron2/val2014"),
    ]:
        image_dir = os.path.join(root, image_dirname)
        gt_dir = os.path.join(root, sem_seg_dirname)
        name = f"coco_2014_{name}_object_all_sem_seg"
        DatasetCatalog.register(
            name, lambda x=image_dir, y=gt_dir: load_sem_seg(y, x, gt_ext="png", image_ext="jpg")
        )
        if "wo_bg" not in name:
            MetadataCatalog.get(name).set(
                image_root=image_dir,
                sem_seg_root=gt_dir,
                evaluator_type="sem_seg",
                ignore_label=255,
                stuff_classes=["background"] + meta["stuff_classes"],
            )
        else:
            MetadataCatalog.get(name).set(
                image_root=image_dir,
...
register_all_coco_object_10k(_root)
