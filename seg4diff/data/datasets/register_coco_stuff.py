...
def _get_coco_stuff_27_meta():
    stuff_ids = [k for k in COCO_27_CATEGORIES]
    assert len(stuff_ids) == 27, len(stuff_ids)

    stuff_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(stuff_ids)}
    stuff_classes = [k for k in COCO_27_CATEGORIES]
...
def register_all_coco_stuff_10k(root):
    ...
    meta_27 = _get_coco_stuff_27_meta()
    for name, image_dirname, sem_seg_dirname in [
        ...
        ("test_object", "val2017", "annotations_object/val2017"),
        ("test_object_wo_bg", "val2017", "annotations_object/val2017"),
        ("test_27", "val2017", "annotations_27_detectron2/val2017"),
        ("train_subset", "train2017", "annotations_detectron2/train2017"),
    ]:
        image_dir = os.path.join(root, image_dirname)
        gt_dir = os.path.join(root, sem_seg_dirname)
        name = f"coco_2017_{name}_stuff_all_sem_seg"
        if "subset" in name:
            caption = json.load(open("datasets/captions/train_caption_coco.json", 'r'))
            caption = {key: value for key, value in caption.items()}
            data = []
            # exclude images with no caption
            for image_id in list(caption.keys()):
                data.append(
                    {
                        "file_name": os.path.join(image_dir, f"{image_id}.jpg"),
                        "sem_seg_file_name": os.path.join(gt_dir, f"{image_id}.png"),
                        "image_id": image_id,
                    }
                )
            
            DatasetCatalog.register(
                name, lambda x=data: data
            )
        else:
