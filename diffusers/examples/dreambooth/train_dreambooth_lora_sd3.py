...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        caption_dir = "/mnt/data3/catseg_dataset/coco/annotations/captions_train2017.json"
        self.caption = json.load(open(caption_dir, 'r'))
        self.caption = {int(item['image_id']): item['caption'] for item in self.caption["annotations"]}
        ...
        if args.dataset_name is not None:
            ...
            instance_images_path = sorted(list(self.instance_data_root.iterdir()))
            for path in instance_images_path:
                instance_images.append(Image.open(path))
                image_id = int(str(path).split("/")[-1].split(".")[0])
                if image_id in self.caption:
                    custom_instance_prompts.append(self.caption[image_id])
